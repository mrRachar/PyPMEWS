...
## TODO # Add escaping everywhere, and make sure it works this time! getting rid of needless extra 'cls.unescaped's
## TODO # CLEAN!
...
class Parser(metaclass=ParserMeta):
    ...
    def countunescaped(needle: str, haystack: str, start: int=0) -> int:
        ...
        for i, letter in enumerate(haystack[start:]):
            if letter == needle and (haystack[i-1] != '\\' if i > 0 else True):
                count += 1
        return count
...
if __name__ == '__main__':
    def tests():
        teststring = 'he(llo)helloavfv(hello)wdf(wdvhello)svhellosfv'
        print('test string: {!r}'.format(teststring))
        print('shallowsplit (hello):', Parser.shallowsplit('hello', '(', ')', teststring))
        print('shallowsplit (wd):', Parser.shallowsplit('wd', '(', ')', teststring))
        print('\nshallowfind (hello):', Parser.shallowfind('hello', '(', ')', teststring))
        print('shallowfind (vhello):', Parser.shallowfind('vhello', '(', ')', teststring))
    #print(', '.join("'{}'".format(x) for x in Parser.repeatrange('2-4')))
    #parsed = Parser.parse('[1-3A-z]hello')
    parsed = Parser.parse(r'[#alpha#012](word[]:m.{3})(word[]:hello){2}he[l]lo')
