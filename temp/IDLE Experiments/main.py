...
## TODO-LONGTERM # final regex in parser to get rid of
## TODO # group negation
...
class Matcher:
    ...
    def negativecompare(self, string: str, node=None) -> Match: ##TEST THIS OUT
        ...
        DEBUG or print('#negcalled', letter, node)

        if not isinstance(node, Join) and self.equals(letter, node):
            DEBUG or print('#firstfault', letter, node)
            return Match()
        ...
        for branch in node.branches:
            if not isinstance(branch, Join) and self.equals(letter, branch):
                print('#branchfault', letter, branch)
                return Match()
        for branch in node.branches:
            ...
            match += branchmatch
            DEBUG or print('#negbranch1', match)
            return match
        else:
            DEBUG or print('#negbranchfail', branch)
            return Match()
